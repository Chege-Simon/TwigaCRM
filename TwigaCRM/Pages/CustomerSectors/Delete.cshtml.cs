...
namespace TwigaCRM.Pages.CustomerSectors
{
    [Authorize]
    public class DeleteModel : PageModel
    {
        ...
        {
            IsPermitted = _checkPermissions.CheckPermission(User, "remove_customer_sector");
            if (!IsPermitted)
            {
            ...
            CustomerSector = await _context.CustomerSector
                .Include(c => c.Customer)
                .Include(c => c.Sector).FirstOrDefaultAsync(m => m.Id == id);

            if (CustomerSector != null)
            {
                _context.CustomerSector.Remove(CustomerSector);
        ...
        public async Task<IActionResult> OnPostAsync(int? id)
        {
            ...
            CustomerSector = await _context.CustomerSector.FindAsync(id);
