...
namespace TwigaCRM.Pages.CropsAndAnimals
{
    ...
    public class EditModel : PageModel
    {
        ...
        {
            IsPermitted = _checkPermissions.CheckPermission(User, "edit_crops_and_animals");
            if (!IsPermitted)
            {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            _context.Attach(CropAndAnimal).State = EntityState.Modified;

            try
            {
                //await _context.SaveChangesAsync();
                await _context.SaveChangesAsync(User?.FindFirst(ClaimTypes.NameIdentifier).Value);
                _toastNotification.Success(CropAndAnimal.Type + " Edited!");
            }
            catch (DbUpdateConcurrencyException)
            {
                if (!CropAndAnimalExists(CropAndAnimal.Id))
                {
                    return NotFound();
                }
                else
        ...
        private bool CropAndAnimalExists(int id)
        {
            return _context.CropAndAnimal.Any(e => e.Id == id);
        }
    }
}
