...
namespace TwigaCRM.Pages.RAMSaleTargets
{
    ...
    public class DetailsModel : PageModel
    {
        ...
        public IList<RAMSaleTargetMapping> RAMSaleTargetMappings { get; set; }
        [BindProperty]
        ...
        {
            IsPermitted = _checkPermissions.CheckPermission(User, "view_RAM_ST_details");
            if (!IsPermitted)
            {
            ...
            RAMSaleTargetMappings = await _context.RAMSaleTargetMapping
                ...
                .Where(t => t.RAMSaleTargetId == RAMSaleTarget.Id).OrderByDescending(s => s.Id).ToListAsync();
            Permissions = await _context.Permission.Include(p => p.AppRolePermissions).ToListAsync();
            ApprovalStatus = RAMSaleTarget.ApprovalStatus;
        ...
        {
            IsPermitted = _checkPermissions.CheckPermission(User, "add_RAM_ST_target");
            if (!IsPermitted)
            {
                return RedirectToPage("/403");
            }
            ...
            if (!ModelState.IsValid)
            {
                ...
                return RedirectToPage("./Details", new { id = RAMSaleTargetMapping.RAMSaleTargetId });
            }
            ...
            _context.RAMSaleTargetMapping.Add(RAMSaleTargetMapping);
            //await _context.SaveChangesAsync();
            await _context.SaveChangesAsync(User?.FindFirst(ClaimTypes.NameIdentifier).Value);
            _toastNotification.Success("RAMSaleTargetMapping Recorded!");
        ...
        {
            IsPermitted = _checkPermissions.CheckPermission(User, "approve_RAM_ST");
            if (!IsPermitted)
            {
                return RedirectToPage("/403");
            }
            ...
            RAMSaleTarget = await _context.RAMSaleTarget.FirstOrDefaultAsync(d => d.Id == id);

            RAMSaleTarget.IsSubmitted = ApprovalStatus != "Rejected" ? true : false;
            RAMSaleTarget.ApprovalStatus = ApprovalStatus;
        ...
        {
            IsPermitted = _checkPermissions.CheckPermission(User, "add_RAM_ST_remarks");
            if (!IsPermitted)
            {
                return RedirectToPage("/403");
            }
            ...
            RAMSaleTarget.Remarks = RAMSaleTarget.Remarks + System.Environment.NewLine + AppUser.FirstName + " " + AppUser.LastName + " ( " + AppUser.AppRole.Title + ") : " + Remarks;
            _context.Attach(RAMSaleTarget).State = EntityState.Modified;
            //await _context.SaveChangesAsync();
            await _context.SaveChangesAsync(User?.FindFirst(ClaimTypes.NameIdentifier).Value);
