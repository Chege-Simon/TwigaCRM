...
namespace TwigaCRM.Pages.FinancialYears
{
    ...
    public class IndexModel : PageModel
    {
        ...
        {
            IsPermitted = _checkPermissions.CheckPermission(User, "view_financial_years");
            if (!IsPermitted)
            {
                return RedirectToPage("/403");
            }
            ViewData["shownav"] = _checkPermissions.NavPermissionAsync(User);
            FinancialYears = await _context.FinancialYear.OrderByDescending(s => s.Id).ToListAsync();
            return Page();
        }
        [BindProperty]
        ...
        {
            IsPermitted = _checkPermissions.CheckPermission(User, "create_financial_year");
            if (!IsPermitted)
            {
                return RedirectToPage("/403");
            }
            ...
            _toastNotification.Success("Financial Year Created!");
